...
def get_model(cfg: OmegaConf) -> nn.Module:
    ...
    elif cfg[KEY].LIBRARY == "semseg":
        model = get_semseg_model(cfg)
    else:
        assert False, "The library is not implemented"
    return model
...
def get_semseg_model(cfg: OmegaConf) -> nn.Module:
    cls = eval(f"{cfg[KEY].VERSION}")
    model = cls(
        backbone=cfg[KEY].ENCODER,
        num_classes=cfg[KEY].NUM_CLASSES
    )
    model.init_pretrained(cfg[KEY].PRETRAINE)

...
class MixVisionTransformerEncoder(MixVisionTransformer, EncoderMixin):
    ...
    def set_in_channels(self, in_channels, *args, **kwargs):
        ...
        if in_channels == 1:
            ...
            new_weight = torch.Tensor(module.out_channels, in_channels // module.groups, *module.kernel_size)

            for i in range(in_channels):
                new_weight[:, in_channels - i - 1] = weight[:, (default_in_channels - i - 1) % default_in_channels]

            new_weight = new_weight * (default_in_channels / in_channels)
            module.weight = nn.parameter.Parameter(new_weight)

...
class Mosaic:
    def __init__(self, img_dir, ann_dir, size=(512, 512), ignore_label=255):
        ...
        os.makedirs(f'{self.img_dir}_9mosaic', exist_ok=True)
        os.makedirs(f'{self.ann_dir}_9mosaic', exist_ok=True)
    ...
    def doMosaic(self, name, indexes):
        ...
        sizes = [
            [ch, cw], [ch, cw], [ch, w - 2 * cw],
            [ch, cw], [ch, cw], [ch, w - 2 * cw],
            [h - 2 * ch, cw], [h - 2 * ch, cw], [h - 2 * ch, w - 2 * cw],
        ]

        from_to = [  # top left h, w to bottom right h, w]
            [0, 0, ch, cw], [0, cw, ch, 2 * cw], [0, 2 * cw, ch, w],
            [ch, 0, 2 * ch, cw], [ch, cw, 2 * ch, 2 * cw], [ch, 2 * cw, 2 * ch, w],
            [2 * ch, 0, h, cw], [2 * ch, cw, h, 2 * cw], [2 * ch, 2 * cw, h, w],
        ]

        trans = [Resize(size_, seg_fill=self.ignore_label) for size_ in sizes]
        out_img = torch.zeros((4, h, w)).cuda()
        out_ann = torch.zeros((1, h, w)).cuda()
        for idx in range(9):
            ft_ = from_to[idx]
        ...
        img_path = os.path.join(f'{self.img_dir}_9mosaic', name + ".tif")
        ann_path = os.path.join(f'{self.ann_dir}_9mosaic', name + ".png")
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(n)):
        indexes_ = [i * 9 + j for j in range(9)]
        mosaic.doMosaic(f"mosaic_9_{i}", indexes=indexes_)

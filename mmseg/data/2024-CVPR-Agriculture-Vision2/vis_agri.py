...
class VisAgri:
    ...
    def __init__(self, ):
        self.seg_local_visualizer = SegLocalVisualizer()
        self.seg_local_visualizer.dataset_meta = dict(
            classes=("background",
                     "double_plant",
                     "drydown",
                     "endrow",
    ...
    def show(self, img_path='img_dir/val/1CJM6XP1B_545-2135-1057-2647.tif', lbl_path='ann_dir/val/1CJM6XP1B_545-2135-1057-2647.png'):

        img_rgb = sio.imread(img_path)[:,:,:3]
        lbl = sio.imread(lbl_path)[None, :, :]
        lbl[lbl==255] = 9

        gt_sem_seg_data = dict(data=torch.from_numpy(lbl).long())
        gt_sem_seg = PixelData(**gt_sem_seg_data)
        gt_seg_data_sample = SegDataSample()
        gt_seg_data_sample.gt_sem_seg = gt_sem_seg
        self.seg_local_visualizer.add_datasample('visualizer_example', img_rgb, gt_seg_data_sample, show=True)


def do_vis(data_root='/home/liuwang/liuwang_data/documents/datasets/seg/'
                     '2024-CVPR-Agriculture-Vision/supervised/Agriculture-Vision-2021-MMSeg-RGBN'
):
    vis = VisAgri()

    names = os.listdir(os.path.join(data_root, 'img_dir/val'))
    ...
    for name in tqdm(names):
        img_path = os.path.join(data_root, 'img_dir/val', name)
        lbl_path = os.path.join(data_root, 'ann_dir/val', name[:-4]+'.png')
        # print(img_path, lbl_path, roi_path)
        ...
        img_rgb_pca, _ = applyPCA(img_rgb, pca)
        img_rgb_pca = np.repeat(img_rgb_pca[:, :, 0:1], repeats=3, axis=-1)
        img_rgb_pca = np.clip(img_rgb_pca / 6 * 128 + 128, a_min=0, a_max=255).astype(np.uint8)
        cv2.imshow('rgb_pca', img_rgb_pca)
        cv2.waitKey(1)
        print(np.unique(img_rgb_pca), img_rgb_pca.max(), img_rgb_pca.min())



        img_rgb_pca1 = np.repeat(img_rgb_pca[:,:,0:1], repeats=3, axis=-1)
        cv2.imshow('rgb_pca1', img_rgb_pca1)
        cv2.waitKey(1)

        img_rgb_pca2 = np.repeat(img_rgb_pca[:, :, 1:2], repeats=3, axis=-1)
        cv2.imshow('rgb_pca2', img_rgb_pca2)
        cv2.waitKey(1)

        img_rgb_pca3 = np.repeat(img_rgb_pca[:, :, -1:], repeats=3, axis=-1)
        cv2.imshow('rgb_pca3', img_rgb_pca3)
        cv2.waitKey(1)

        img_nir = sio.imread(img_path)[:,:,-1:]

        img_nir = np.repeat(img_nir, repeats=3, axis=-1)
        cv2.imshow('nir', img_nir)
        cv2.waitKey(1)
        vis.show(img_path, lbl_path)


# Press the green button in the gutter to run the script.
if __name__ == '__main__':
    # do_vis(data_root='../data/WaterSeg/val')
    # do_vis(data_root='../data/WaterSeg/train_mosaic')
    # do_vis(data_root='../data/WaterSeg/train')
    do_vis()
